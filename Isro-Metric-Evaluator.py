...
TASK 2 - Registration Deliverables and Metric Evaluator
Standard evaluation compliance metrics:
...
try:
    from .subpixel_uniformity import compute_spatial_uniformity_metrics
    from .geometry_warping import warp_lunar_image, estimate_robust_transformation
except ImportError:
    from subpixel_uniformity import compute_spatial_uniformity_metrics
    from geometry_warping import warp_lunar_image, estimate_robust_transformation
...
def compute_reprojection_errors(pts0: np.ndarray, pts1: np.ndarray, H: np.ndarray) -> np.ndarray:
    ...
    pts0_t = cv2.perspectiveTransform(pts0.reshape(-1, 1, 2).astype(np.float32), H).reshape(-1, 2)
...
def compute_isro_metrics(
    ...
    """
    Computes complete ISRO PS metrics for Task 2
    """
    ...
    # Uniformity - ISRO explicitly asks uniform distribution
    # Check 3x3 (PS primary), 6x6, 8x8
    ...
    metrics = {
        ...
        "success_rate_2px": round(ratio_at(2.0), 4),  # PS calls this success rate <2px
        "inlier_count_3px": count_at(3.0),
        "inlier_ratio_3px": round(ratio_at(3.0), 4),

        # RMSE metrics (PS: sub-pixel accuracy)
        ...
        # Uniformity metrics (PS: uniform spatial distribution)
        ...
        # ISRO final PASS/FAIL (explicit in PS: sub-pixel + uniform)
        ...
        "method_refinement": "Inverse-Compositional Lucas-Kanade + cornerSubPix",
        "method_uniformity": "Quadtree binning 8x8 + ANMS"
    }
...
def save_isro_deliverables(
    ...
    H = np.array(pipeline_result["H_scaled"]) if pipeline_result["H_scaled"] is not None else None
    ...
    if H is not None and len(pts0_in) > 0:
        pts0_in_t = cv2.perspectiveTransform(pts0_in.reshape(-1, 1, 2).astype(np.float32), H).reshape(-1, 2)
        residuals = np.linalg.norm(pts0_in_t - pts1_in, axis=1)
    else:
        residuals = np.array([])

    # Conf - pipeline may have dropped it after binning, use 1.0
    conf = np.ones(len(pts0), dtype=np.float32)

    # Compute full metrics
    metrics = compute_isro_metrics(pts0, pts1, pts0_in, pts1_in, H, img0_scaled.shape, residuals, conf)
    ...
    if H is not None:
        ...
        with open(os.path.join(out_dir, "H_matrix.txt"), "w") as f:
    ...
    with open(os.path.join(out_dir, "isro_metrics.json"), "w") as f:
        json.dump(metrics, f, indent=2, default=lambda x: x.tolist() if isinstance(x, np.ndarray) else float(x) if isinstance(x, (np.float32, np.float64)) else x)
    ...
    print(f" Coverage 6x6           : {metrics['uniformity_coverage_6x6']*100:.1f}%")
    print(f" Coverage 8x8           : {metrics['uniformity_coverage_8x8']*100:.1f}%")
    print(f" Robust Method          : {metrics['method_robust']}")
    print(f" ISRO PASS              : {'✅ YES' if metrics['isro_pass'] else '❌ NO'}")
    print("=" * 78 + "\n")
